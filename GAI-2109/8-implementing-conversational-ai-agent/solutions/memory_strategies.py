"""
Lab Solution: Memory Management Strategies
...
class SlidingWindowMemory:
    ...
    def apply(self, history: List[ModelMessage]) -> List[ModelMessage]:
        ...
        # Keep system messages + recent exchanges. Pydantic AI does NOT
        # expose a top-level .role attribute on ModelMessage — system
        # prompts live inside .parts as SystemPromptPart instances. Walk
        # parts to find them rather than checking m.role (which is always
        # absent and would silently filter every message out).
        system_msgs = [
            m
            for m in history
            if any(isinstance(p, SystemPromptPart) for p in getattr(m, "parts", []))
        ]
        recent_msgs = history[-self.window_size :]
