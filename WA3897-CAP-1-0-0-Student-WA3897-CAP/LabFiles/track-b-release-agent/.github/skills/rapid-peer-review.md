...
produces a written findings document that a release manager or procurement officer
can act on directly.
...
**Pass**, **Needs Attention**, or **Fail**, and write 1–3 sentences of findings.
...
Scope Constraints). Check whether `mock_data/`, functional pipeline modules
(`runner.py`, `deploy.py`, `changelog_reader.py`), or `pyproject.toml` were
...
- Data access only through `data/loader.py`, not direct file reads in agent or tools
- Agent logic only in `agent.py`; tool logic only in `tools/`
- Models defined only in `models.py`
- Stub implementations (`dependency_scanner.py`, `ticket_client.py`) do not violate
  the interface contracts defined in `openspec/pipeline-architecture.md`
- Functional pipeline modules (`runner.py`, `deploy.py`, `changelog_reader.py`) are
  unchanged unless explicitly permitted
- All tool functions have docstrings (required by `AGENTS.md`)
- Type hints and `from __future__ import annotations` present in all new modules
...
- `ReleaseRecommendation.decision` is always `release`, `hold`, or `escalate`
- `ReleaseRecommendation.rationale` is always a non-empty string
- Tool errors are caught and surfaced in the recommendation, not silently swallowed
- The agent does not call `pipeline/deploy.py` directly (advisory role only)
...
**Project**: Release Readiness and Dependency Risk Agent (Track B)
