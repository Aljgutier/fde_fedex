...
Each menu card SHALL display a size selector showing all available sizes (small, medium, large) with pricing that reflects the active pricing tier. During peak hours, prices display normally. During off-peak hours, each size option SHALL show the original price with a strikethrough and the discounted price beside it. A pricing tier indicator SHALL be visible on the menu showing whether peak or off-peak pricing is active.
...
The order button price SHALL reflect the active pricing tier. During off-peak hours, the button SHALL show the discounted base price plus any full-price customization surcharges.
